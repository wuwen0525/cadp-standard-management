...
function renderDashboard(){
  ...
  $('#taskList').innerHTML=topMissing.map(item=>{const project=projects.find(p=>p.id===item.projectId);return `<button class="task-line" data-missing-link="${item.id}"><span>!</span><div><strong>${item.material}</strong><small>${project?.name||'未知项目'} · ${item.category}</small></div><em>待确认</em></button>`}).join('')||'<div class="empty-state">暂无待处理材料</div>';
...
function renderProjectTable(){
  ...
  $('#projectRows').innerHTML=filtered.map(project=>`<div class="table-row"><span class="project-name"><i>标</i><span><strong>${project.name}</strong><small>${project.code||'暂未编号'} · ${project.files||0} 个目录文件</small></span></span><span><i class="project-status">${stages[project.current-1].name}</i></span><span>${project.owner||'待补充'}</span><span><b class="material-count ${projectMissing(project.id).length?'has-missing':''}">${projectMissing(project.id).length?`${projectMissing(project.id).length} 待确认`:'完整'}</b></span><span>${progressFor(project)}%</span><span class="row-actions"><button data-open="${project.id}">办理</button><button data-edit="${project.id}">修改</button></span></div>`).join('')||'<div class="empty-state">没有找到符合条件的项目</div>';
...
function renderProjectMenu(){
  $('#projectMenu').innerHTML=projects.map(p=>`<button data-project-id="${p.id}"><strong>${p.name}</strong><small>${stages[p.current-1].name} · ${p.code||'暂未编号'}</small></button>`).join('');
...
function renderStageDetail(){
  ...
  if($('#returnStage'))$('#returnStage').addEventListener('click',()=>{const target=selectedProject.current===11?10:Math.max(1,selectedProject.current-1);selectedProject.current=target;selectedStage=target;save('cadp-projects-v3',projects);addActivity(selectedProject.name,`退回至“${stages[target-1].name}”`,'已退回');renderWorkflow();showToast('项目已退回修改','已退回')});
}

function renderActivities(){
  $('#activityRows').innerHTML=activities.map(row=>`<div class="table-row"><span>${row.time}</span><span>${row.project}</span><span>${row.action}</span><span>${row.operator}</span><span><i class="result-tag ${row.result==='已退回'?'returned':''}">${row.result}</i></span></div>`).join('')||'<div class="empty-state">暂无办理记录</div>';
}
...
function updateMissing(id,state){const item=missingItems.find(i=>i.id===id);if(!item)return;item.state=state;save('cadp-missing-v2',missingItems);addActivity(projects.find(p=>p.id===item.projectId)?.name||'项目',`${item.material}：${state==='ignored'?'忽略误报':state==='resolved'?'补充完成':'重新打开'}`);renderMissing();showToast(state==='ignored'?'已忽略该条自动判断':state==='resolved'?'材料已标记补充':'已重新打开')}
...
function renderSystem(){
  ...
  $('#roadmapList').innerHTML=roadmap.map(item=>`<label class="roadmap-item"><input type="checkbox" data-roadmap="${item.id}" ${item.done?'checked':''}><span></span><div><strong>${item.title}</strong><small>${item.owner} · 计划 ${item.due}</small></div></label>`).join('');
  $$('[data-roadmap]').forEach(input=>input.addEventListener('change',()=>{const item=roadmap.find(r=>r.id===Number(input.dataset.roadmap));item.done=input.checked;save('cadp-roadmap-v2',roadmap);renderSystem();showToast(input.checked?'体系任务已完成':'体系任务已恢复')}));
  renderStandardRows();
}

function renderStandardRows(){const q=$('#standardSearch').value.trim().toLowerCase();const list=publishedStandards.filter(s=>`${s[0]}${s[1]}`.toLowerCase().includes(q));$('#standardRows').innerHTML=list.map(s=>`<div class="table-row"><span><strong>${s[0]}</strong></span><span>${s[1]}</span><span>${s[0].slice(-4)}</span><span><i class="project-status">${categoryFor(s[1])}</i></span></div>`).join('')}
...
$('#newProjectForm').addEventListener('submit',event=>{event.preventDefault();const d=Object.fromEntries(new FormData(event.target));const project={id:Date.now(),name:d.name,code:d.code||'提案项目',owner:d.owner,due:d.due,current:1,status:'在研',files:0,notes:d.notes};projects.unshift(project);save('cadp-projects-v3',projects);selectedProject=project;selectedStage=1;$('#projectDialog').close();addActivity(project.name,'创建项目并进入提案环节');buildFilters();renderDashboard();navigate('workflow');showToast('新项目已创建')});
$('#editProjectForm').addEventListener('submit',event=>{event.preventDefault();const d=Object.fromEntries(new FormData(event.target)),project=projects.find(p=>p.id===Number(d.id));Object.assign(project,{name:d.name,code:d.code,owner:d.owner,due:d.due,current:Number(d.current),status:d.status,notes:d.notes});save('cadp-projects-v3',projects);if(selectedProject.id===project.id){selectedProject=project;selectedStage=project.current}$('#editDialog').close();addActivity(project.name,'修改项目基本信息');renderProjectTable();showToast('项目信息已保存')});
$('#advanceStage').addEventListener('click',()=>{if(selectedProject.current>=stages.length)return;const before=stages[selectedProject.current-1].name;selectedProject.current+=1;selectedStage=selectedProject.current;if(selectedProject.current===stages.length)selectedProject.status='已发布';save('cadp-projects-v3',projects);addActivity(selectedProject.name,`完成“${before}”，进入“${stages[selectedProject.current-1].name}”`);renderWorkflow();showToast('已推进到下一环节')});
$('#missingFile').addEventListener('change',e=>{$('#missingSelectedFile').textContent=e.target.files[0]?`已选择：${e.target.files[0].name}`:''});
$('#confirmUpload').addEventListener('click',()=>{if(!$('#missingFile').files[0]){showToast('请先选择文件','尚未选择');return}const item=missingItems.find(i=>i.id===uploadMissingId);item.filename=$('#missingFile').files[0].name;$('#uploadDialog').close();updateMissing(uploadMissingId,'resolved')});
$('#clearActivities').addEventListener('click',()=>{activities=[];save('cadp-activities-v2',activities);renderActivities();showToast('演示办理记录已清理')});
...
$('#addSystemItem').addEventListener('click',()=>{const title=prompt('请输入体系建设任务：');if(!title)return;roadmap.push({id:Date.now(),title,owner:'待指定',due:'待确定',done:false});save('cadp-roadmap-v2',roadmap);renderSystem();showToast('体系任务已添加')});
...
buildFilters(); renderDashboard(); renderProjectTable(); renderMissing(); renderSystem(); renderActivities();
